...
def deleteIngredient(id):
    exec_commit('DELETE FROM ingredients WHERE ingredient_id = %s', [id])


def editName(id, name):
    update_sql = 'UPDATE ingredients SET ingredient_name = %s WHERE ingredient_id = %s'
    exec_commit(update_sql, [name, id])


def editAisle(id, aisle):
    update_sql = 'UPDATE ingredients SET aisle = %s WHERE ingredient_id = %s'
    exec_commit(update_sql, [aisle, id])


def editIngredient(id, name, aisle):
    update_sql = 'UPDATE ingredients SET ingredient_name = %s, aisle = %s WHERE ingredient_id = %s'
